...
interface PortafoliosClientProps {
    studioSlug: string;
    initialPortfolios: StudioPortfolio[];
}

export function PortafoliosClient({
    studioSlug,
    initialPortfolios,
}: PortafoliosClientProps) {
    const router = useRouter();
    const [portfolios, setPortfolios] = useState<StudioPortfolio[]>(() => {
        // Ordenar por order al inicializar
        return [...initialPortfolios].sort((a, b) => a.order - b.order);
    });
    ...
    // Sincronizar portfolios cuando cambian desde el servidor
    useEffect(() => {
        // Solo sincronizar si NO estamos navegando
        if (!isNavigatingRef.current) {
            const sortedPortfolios = [...initialPortfolios].sort((a, b) => a.order - b.order);
