...
export function CurrentPlanCard({ data, studioSlug }: CurrentPlanCardProps) {
    ...
    // Cancelar suscripción - Redirigir a Stripe Customer Portal
    const handleCancelSubscription = async () => {
        setLoadingCancel(true);
        try {
            const result = await createCustomerPortal(studioSlug);
            if (result.success && result.url) {
                // Redirigir a Stripe Portal donde puede cancelar
