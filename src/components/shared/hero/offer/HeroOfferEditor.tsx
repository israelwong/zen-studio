...
export default function HeroOfferEditor({
    ...
    // Filtrar tabs según contexto - en portfolios NO mostrar tab de botones
    const allTabs = [
    ...
    const tabs = useMemo(() => {
        if (effectiveContext === 'portfolio') {
            // En portfolios, ocultar tab de botones (no generar tráfico, solo presentar)
            return allTabs.filter(tab => tab.id !== 'botones');
        }
        return allTabs;
    }, [effectiveContext]);

    // Cambiar tab automáticamente si el activo no está disponible en el contexto actual
    useEffect(() => {
        const isCurrentTabAvailable = tabs.some(tab => tab.id === activeTab);
        if (!isCurrentTabAvailable) {
            // Si el tab actual no está disponible (ej: botones en portfolio), cambiar a informacion
                ...
                <div className="space-y-4">
                    ...
                    {activeTab === 'informacion' && (
                        <div className="space-y-4">
                            <ZenInput
                                ...
                                placeholder={effectiveContext === 'portfolio' ? 'Título de tu portafolio' : 'Título principal'}
                            />
                            ...
                            <ZenInput
                                ...
                                placeholder={effectiveContext === 'portfolio' ? 'Categoría o especialidad' : 'Subtítulo opcional'}
                            />
                            ...
                            <ZenTextarea
                                ...
                                placeholder={effectiveContext === 'portfolio' ? 'Descripción breve de tu portafolio' : 'Descripción limitada a 100 caracteres'}
                                maxLength={100}
                                minRows={3}
                            />
                        </div>
                    ...
                    {activeTab === 'fondo' && (
                        <div className="space-y-4">
                            ...
                            {/* Controles de Parallax - Solo mostrar si no es video */}
