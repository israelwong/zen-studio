...
export function GoogleDriveDisconnectModal({
  ...
  const [limpiarPermisos, setLimpiarPermisos] = useState(canRevokePermissions);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="bg-zinc-900 border-zinc-800 max-w-md" overlayZIndex={10100}>
          ...
          <div className="py-4">
            <div className="p-4 rounded-lg border bg-zinc-800/50 border-zinc-700">
              <div className="text-sm text-zinc-300">
                Al desconectar Google Drive:
              </div>
              <ul className="mt-2 space-y-1 text-xs text-zinc-400 list-disc list-inside">
                <li>Se detendrá la sincronización automática</li>
                <li>Los permisos públicos de las carpetas se mantendrán</li>
                <li>Los entregables vinculados seguirán funcionando</li>
              </ul>
            </div>
          </div>
        )}
        ...
        <DialogFooter className="gap-2">
          ...
          >
            {canRevokePermissions && limpiarPermisos ? 'Revocar y Desconectar' : 'Desconectar'}
          </ZenButton>
        </DialogFooter>
      </DialogContent>
    </Dialog>
